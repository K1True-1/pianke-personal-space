...
> **待更新版本 · 夜色画廊 V1（2026-09-12）**：本分支保存首版动效设计，供版本管理和后续对比。尚未发布，浏览器验收未通过；下一版将增强动效表现和整体视觉层次。详见 [版本记录](CHANGELOG.md) 与 [检查记录](design-qa.md)。
...
新版采用深色画廊布局、森林光影背景和浅黄绿色重点色。照片带有轻微的鼠标视差、悬停边缘光和入场动画；分类导航、作品卡片及弹窗提供过渡反馈。手机端使用顶部导航和单列相册。

照片视差只在支持鼠标悬停的设备上启用；如果系统开启「减少动态效果」，页面会关闭动画和位移过渡。小说正文保持稳定，方便长时间阅读。

这些改动目前保存在源码中；线上网站会在完成浏览器检查并重新发布后更新。
...
- 键盘操作：Tab 进入日历，上下方向键移动一天，左右方向键移动一周，Home / End 跳到首日 / 今天；Enter 或空格选中日期。
- 活动总数增加时，今天的方格会短暂亮起；这只是视觉反馈，不额外增加活动次数。
...
- 森林光影背景：使用 OpenAI ImageGen 为本项目生成，文件为 `public/gallery-atmosphere.png`。
